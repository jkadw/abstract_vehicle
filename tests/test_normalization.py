"""Tests for the normalization layer."""

from __future__ import annotations

from custom_components.vehicle.model import CapabilitySupport, VehicleCapabilities, VehicleState
from custom_components.vehicle.normalization import (
    NormalizationConfig,
    infer_capabilities,
    normalize_openings,
    normalize_vehicle_data,
    normalize_vehicle_state,
)


def test_normalize_vehicle_state_precedence() -> None:
    """Charging and error precedence should be preserved."""

    assert (
        normalize_vehicle_state(
            {
                "status": "driving",
                "charging_active": True,
                "backend_online": True,
            }
        )
        is VehicleState.CHARGING
    )
    assert (
        normalize_vehicle_state(
            {
                "status": "charging",
                "has_error": True,
            }
        )
        is VehicleState.ERROR
    )
    assert (
        normalize_vehicle_state({"status": "offline", "available": False})
        is VehicleState.OFFLINE
    )
    assert normalize_vehicle_state({}) is VehicleState.UNKNOWN


def test_normalize_openings_aggregates_any_open_window() -> None:
    """Window aggregation should collapse many openings into one flag."""

    assert normalize_openings({"front_left": "closed", "sunroof": "open"}) is True
    assert normalize_openings({"front_left": "closed", "sunroof": "closed"}) is False
    assert normalize_openings(None) is None


def test_infer_capabilities_preserves_actions_and_fills_state_support() -> None:
    """Normalization should infer readable state support from raw data."""

    declared = VehicleCapabilities(
        lock=CapabilitySupport(state_supported=False, action_supported=True),
        climate=CapabilitySupport(state_supported=False, action_supported=True),
    )

    inferred = infer_capabilities(
        raw_state={
            "locked": True,
            "climate_active": False,
            "charging_plugged": True,
        },
        raw_metrics={
            "battery_level": 80.0,
            "odometer": 1234.5,
            "openings": {"sunroof": "closed"},
            "latitude": 1.0,
            "longitude": 2.0,
        },
        declared_capabilities=declared,
    )

    assert inferred.lock.state_supported is True
    assert inferred.lock.action_supported is True
    assert inferred.windows.state_supported is True
    assert inferred.windows.action_supported is False
    assert inferred.location.state_supported is True
    assert inferred.battery.state_supported is True
    assert inferred.charging.state_supported is True


def test_normalize_vehicle_data_converts_distance_units() -> None:
    """Distance normalization should convert km to mi when configured."""

    normalized = normalize_vehicle_data(
        raw_state={
            "vehicle_id": "vehicle-1",
            "name": "Family EV",
            "manufacturer": "Mock Motors",
            "model": "Atlas",
            "vehicle_type": "ev",
            "status": "parked",
            "locked": True,
            "climate_active": False,
            "charging_active": False,
            "charging_plugged": True,
        },
        raw_metrics={
            "range": 100.0,
            "odometer": 10.0,
            "battery_level": 90.0,
            "openings": {"front_left": "closed"},
            "source_units": {"distance_unit": "km"},
        },
        capabilities=VehicleCapabilities(
            lock=CapabilitySupport(state_supported=True, action_supported=True)
        ),
        config=NormalizationConfig(distance_unit="mi"),
    )

    assert normalized.state is VehicleState.PARKED
    assert normalized.range == 62.137
    assert normalized.odometer == 6.214
    assert normalized.capabilities.lock.action_supported is True
    assert normalized.capabilities.windows.state_supported is True
