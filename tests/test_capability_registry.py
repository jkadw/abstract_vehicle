...
def test_button_rule_map_exposes_registry_driven_service_names() -> None:
    ...
    assert rules["start_heating"][0] == "climate"
    assert rules["start_heating"][1].action == "start_heating"
    assert rules["start_cooling"][1].action == "start_cooling"
    assert rules["start_ev_charging"][0] == "ev_charging"
    assert rules["start_ev_charging"][1].action == "start"
    assert rules["stop_ev_charging"][1].action == "stop"
    assert rules["vehicle_alert_flash"][0] == "vehicle_alert"
    assert rules["vehicle_alert_flash"][1].action == "flash"
    assert rules["vehicle_alert_honk"][1].action == "honk"
    assert rules["vehicle_alert_alarm"][1].action == "alarm"
    assert rules["vehicle_alert_stop"][1].action == "stop"
